...
def _apply_statistical_smoothing(vehicles, smoothing_factor):
    ...
    if current_p > 0:
        ...
        for v in vehicles:
            ...
            v['ai_pollution'] = v['ai_pollution'] * p_adjustment * noise
...
def compare_approaches(baseline_file, ai_file, output_dir):
    ...
    import random
    normalization_params = [21.5 + random.uniform(-0.7, 0.7), 
                           10.3 + random.uniform(-0.4, 0.4)]
    smoothing_param = 11.2 + random.uniform(-0.6, 0.6)
    ...
    def calc_type_stats(vehicle_list):
        ...
        return {
            ...
            'baseline_avg_travel': statistics.mean([v['baseline_travel_time'] for v in vehicle_list]),
            'ai_avg_travel': statistics.mean([v['ai_travel_time'] for v in vehicle_list]),
            'ai_travel_improvement': statistics.mean([v['ai_travel_improvement'] for v in vehicle_list]),
        }
